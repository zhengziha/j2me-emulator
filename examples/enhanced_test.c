...
void test_class_loader(void) {
    ...
    if (!nonexistent) {
        LOG_DEBUG("✓ 不存在的类正确返回NULL\n");
    } else {
        LOG_DEBUG("⚠ 不存在的类错误返回非NULL\n");
    }
...
void test_bytecode_info(void) {
    ...
    LOG_DEBUG("✓ 指令长度: BIPUSH=%d, SIPUSH=%d, RETURN=%d\n", len1, len2, len3);
