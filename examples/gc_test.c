...
int main() {
    ...
    j2me_gc_t* gc = vm->gc;
    if (!gc) {
        LOG_DEBUG("错误: 垃圾回收器未初始化\n");
        j2me_vm_destroy(vm);
        return 1;
    }
    
    LOG_DEBUG("虚拟机和垃圾回收器初始化成功\n\n");
...
void test_gc_basic_allocation(j2me_gc_t* gc) {
    ...
    if (!ptr1) {
        ...
        LOG_DEBUG("  ✓ 基本分配成功: %p (100 bytes)\n", ptr1);
    }
    
    // 测试2: 大对象分配
    void* ptr2 = j2me_gc_allocate(gc, 10000, TEST_OBJECT_TYPE_ARRAY);
    if (!ptr2) {
        LOG_DEBUG("  错误: 大对象分配失败\n");
        test_passed = false;
    } else {
        LOG_DEBUG("  ✓ 大对象分配成功: %p (10000 bytes)\n", ptr2);
    }
    ...
    LOG_DEBUG("  堆使用情况: %zu/%zu bytes (%.1f%%)\n", 
...
void test_gc_root_management(j2me_gc_t* gc) {
    ...
    if (result != J2ME_SUCCESS) {
        LOG_DEBUG("  错误: 垃圾回收失败: %d\n", result);
        test_passed = false;
    }
    
    // 检查根对象是否仍然存在
    if (obj != NULL) {
        LOG_DEBUG("  ✓ 根对象在GC后仍然存在\n");
    } else {
        LOG_DEBUG("  错误: 根对象被错误回收\n");
        test_passed = false;
    }
...
void test_gc_fragmentation(j2me_gc_t* gc) {
    ...
    if (large_obj) {
        LOG_DEBUG("  ✓ 大对象分配成功: %p (8192 bytes)\n", large_obj);
    } else {
        LOG_DEBUG("  警告: 大对象分配失败，可能存在碎片问题\n");
    }
    
    // 打印堆使用情况
    size_t used, free, total;
    j2me_gc_get_heap_info(gc, &used, &free, &total);
    LOG_DEBUG("  最终堆使用: %zu/%zu bytes (%.1f%%)\n", 
           used, total, (double)used * 100.0 / total);
...
void test_gc_stress(j2me_gc_t* gc) {
    ...
    if (successful_allocations > stress_iterations * 0.8) {
        LOG_DEBUG("  ✓ 压力测试通过，系统稳定\n");
    } else {
        LOG_DEBUG("  警告: 分配成功率较低\n");
        test_passed = false;
    }
