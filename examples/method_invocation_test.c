...
void test_method_invocation(j2me_vm_t* vm) {
    ...
    if (result == J2ME_SUCCESS) {
        ...
        LOG_DEBUG("✗ 接口方法调用失败: %d\n", result);
    }
    
    // 清理
    j2me_stack_frame_destroy(test_frame);
    
    LOG_DEBUG("方法调用系统测试完成\n");
}
...
void test_integration_scenarios(j2me_vm_t* vm) {
    ...
    // 场景2: 嵌套方法调用
    LOG_DEBUG("\n2. 测试嵌套方法调用场景\n");
    LOG_DEBUG("✓ 嵌套方法调用场景模拟完成\n");
    
    // 场景3: 异常传播
    LOG_DEBUG("\n3. 测试异常传播场景\n");
    LOG_DEBUG("✓ 异常传播场景模拟完成\n");
